...
const RenderUser = ({ isEdit, toggleEdit }) => {
  const { data, loading, error } = useQuery(GET_BARANG);
  ...
  return data.barang.map((barang, index) => {
    return (
      <tr className="bg-white border-b  ">
        ...
        <td className="py-4 px-6 text-center">{barang.qty}</td>
        <NumericFormat
          value={barang.price}
          displayType={"text"}
        ...
        <td className="py-4 px-6">
          <div className="flex justify-around">
            <span className="text-green-500" onClick={() => toggleEdit(index)}>
              {isEdit ? (
                <svg
            ...
            <span className="text-red-500" onClick={() => remove(barang.id)}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
